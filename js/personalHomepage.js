...
function fun(){
    if (!isShow) {
        ...
        $("#dynamic").text("照片");
        $(".issue_p").text("拍一张照片");
        $(".section_text_box_one").css("display","block");
        $(".section_text_box_two").css("display","none");
        isShow = true;
    }else{
        ...
        $(".section_text_box_two").css("display","block");
        $(".section_text_box_one").css("display","none");
        isShow = false;
    }
}
$.ajax({
...
for(var i = 0; i < 11; i++){
    ...
    img6[0].addEventListener('touchstart',fun1,false);
}
var isshow = false;
function fun1(event){
    switch(event.type){
